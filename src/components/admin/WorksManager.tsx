...
const serviceKeys = ['diagnostics', 'repair', 'adjustment', 'replacement', 'cleaning'];
...
export default function WorksManager() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (!formData.title || !formData.customerInitials || !formData.vehicleMake) {
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
      <Paper sx={{ p: 4, borderRadius: 3, boxShadow: 3 }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
          <Typography variant="h5" sx={{ color: '#004975', fontWeight: 'bold' }}>
            {t('title', { count: works.length })}
          </Typography>
          <PrimaryButton startIcon={<AddIcon />} onClick={() => handleOpenDialog()}>
            {t('addNew')}
          </PrimaryButton>
        </Box>
        ...
        <TableContainer component={Paper} sx={{ borderRadius: 2 }}>
          <Table>
            <TableHead sx={{ backgroundColor: '#F8F9FA' }}>
              <TableRow>
                <TableCell><strong>{t('table.title')}</strong></TableCell>
                <TableCell><strong>{t('table.vehicle')}</strong></TableCell>
                <TableCell><strong>{t('table.customer')}</strong></TableCell>
                <TableCell><strong>{t('table.date')}</strong></TableCell>
                <TableCell><strong>{t('table.status')}</strong></TableCell>
                <TableCell><strong>{t('table.locale')}</strong></TableCell>
                <TableCell align="center"><strong>{t('table.actions')}</strong></TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {works.map((work) => (
                <TableRow key={work.id} hover>
                  ...
                  <TableCell>{work.vehicleMake} {work.vehicleModel}</TableCell>
                  ...
                  <TableCell>
                    <Chip
                      label={t(`form.status${work.status.charAt(0).toUpperCase() + work.status.slice(1)}` as any, work.status)}
                      size="small"
                  ...
                  <TableCell align="center">
                    <IconButton onClick={() => handleOpenDialog(work)} color="primary">
                      <EditIcon />
                    </IconButton>
                    <IconButton onClick={() => handleDelete(work.id!)} color="error">
                      <DeleteIcon />
                    </IconButton>
                  </TableCell>
                </TableRow>
        ...
        <DialogContent>
          <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale={locale}>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
              <Typography variant="subtitle1" sx={{ fontWeight: 'bold', color: '#004975' }}>
                {t('form.groupBasic')}
              </Typography>
              <TextField
                label={t('form.title')}
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    label={t('form.locale')}
                    onChange={(e) => setFormData({ ...formData, locale: e.target.value as 'uk' | 'en' })}
                  >
                ...
                <FormControl fullWidth>
                  ...
                  >
                    ...
                    <MenuItem value="published">{t('form.statusPublished')}</MenuItem>
                    <MenuItem value="archived">{t('form.statusArchived')}</MenuItem>
                  </Select>
                </FormControl>
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                <TextField
                  ...
                  onChange={(e) => setFormData({ ...formData, vinHashed: e.target.value })}
                  fullWidth
                />
              </Box>
              <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                <TextField
                  ...
                  onChange={(e) => setFormData({ ...formData, injectorId: e.target.value })}
                  fullWidth
                />
              </Box>
              <Box sx={{ display: 'flex', gap: 2 }}>
                <DatePicker
                  ...
                  onChange={(newVal) => setFormData({ ...formData, workDate: newVal || dayjs() })}
                ...
                <Autocomplete
                  ...
                  onChange={(e, newValue) => setFormData({ ...formData, services: newValue })}
                  renderInput={(params) => <TextField {...params} label={t('form.services')} />}
                  fullWidth
                />
              </Box>
