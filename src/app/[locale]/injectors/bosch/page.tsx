...
import { boschInjectors } from '@/data/injectors/bosch';
...
export default function BoschInjectorsPage() {
  ...
  const uniquePressures = useMemo(() => {
    const pressures = boschInjectors.map(inj => inj.specifications.pressure);
    return ['all', ...Array.from(new Set(pressures))];
  }, []);

  const uniqueVehicleBrands = useMemo(() => {
    const brands = new Set<string>();
    boschInjectors.forEach(inj => {
  ...
  const filteredInjectors = useMemo(() => {
    return boschInjectors.filter(injector => {
      const matchesSearch = 
