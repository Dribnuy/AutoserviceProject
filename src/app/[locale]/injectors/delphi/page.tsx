...
export default function DelphiInjectorsPage() {
  ...
  const [delphiManufacturerId, setDelphiManufacturerId] = useState<string>("");

  useEffect(() => {
    const findDelphiManufacturer = async () => {
      try {
        ...
        if (!manufacturersSnapshot.empty) {
          const delphiDoc = manufacturersSnapshot.docs[0];
          setDelphiManufacturerId(delphiDoc.id);
        } else {
          setError("Виробника Delphi не знайдено в базі даних");
          setLoading(false);
        }
      } catch (err: any) {
    ...
    findDelphiManufacturer();
  }, []);

  useEffect(() => {
    if (!delphiManufacturerId) return;
    ...
    const loadInjectors = async () => {
      try {
        ...
        const q = query(
          ...
          where("manufacturerId", "==", delphiManufacturerId)
        );
    ...
    loadInjectors();
  }, [delphiManufacturerId]);

  const uniquePressures = useMemo(() => {
    const pressures = injectors
      .map((inj) => inj.specifications?.pressure)
      .filter(Boolean);
        ...
        <Container maxWidth="lg">
          <Box sx={{ mb: 4 }}>
            <Typography variant="body2" color="text.secondary">
              <Link
                href={`/${locale}`}
                style={{ textDecoration: "none", color: "#004975" }}
              >
                {t("nav.home")}
              </Link>
              {" / "}
          ...
          <Typography variant="h6" sx={{ mb: 3, color: "#666" }}>
            {t("delphiPage.foundPrefix")} {filteredInjectors.length}{" "}
          ...
          <Box sx={{ display: "flex", flexWrap: "wrap", m: -1.5 }}>
            {filteredInjectors.map((injector) => {
              const displayName =
                locale === "en" ? injector.nameEn : injector.name;
                ...
                >
                  ...
                  >
                    <CardMedia
                      ...
                      height="200"
                      image={
                        injector.image || "/images/injectors/placeholder.jpg"
                      }
                      alt={displayName}
                    ...
                    >
                      ...
                      {injector.specifications && (
                        <Box sx={{ mb: 2 }}>
                          ...
                          >
                            <strong>{t("delphiPage.specPressure")}</strong>{" "}
                          ...
                          <Typography
                            ...
                            sx={{ display: "block", color: "#666", mb: 0.5 }}
                          >
                            <strong>{t("delphiPage.specFlow")}</strong>{" "}
                            {injector.specifications.flowRate}
                          </Typography>
                          <Typography
                            variant="caption"
                            sx={{ display: "block", color: "#666" }}
                          >
                            <strong>{t("delphiPage.specVoltage")}</strong>{" "}
                            {injector.specifications.voltage}
                          </Typography>
                        </Box>
                      )}
